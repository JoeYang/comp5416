...

{
  ...
  for (i = 0; i < BATCH_HOSTS; ++i) {
    if (narr % batch_interval == i) {
      //printf("BATCH HOST: %d %d\n", i, batch_interval);
      num_packets = batch_size;
      batch_arrival = 1;
      break;
    }
    else {
      //printf("No Batch host: %d\n", i);
      num_packets = 1;
      batch_arrival = 0;
    }
  }
...
{ 
  ...
  scanf("%d", &iar);
  printf("\nenter the max buffer size (octets)\n");
  scanf("%d", &buffer_size);
  ...
  q_len = 0;
  batch_size = iar/BAR;
  batch_interval = pow(iar/BAR, 2) + (BATCH_HOSTS - 1);
  iar = (BATCH_HOSTS * 10) + ((NUM_HOSTS - BATCH_HOSTS) * iar);
  iat = 1.0 / iar;
