...
int
  ...
  r_capacity,        /* gateway processing capacity */
  buffer_size,       /* buffer size to hold packets */		
  mean_pkt_length = 1000,   /* mean packet length */
...

{
  ...
  double utilisation = (q_len * 8)/(iar * pow(10.0, 6.0));
  //printf("Utilisation: %d %f\n", q_len, utilisation);
  if (utilisation <= 0.9) {
	/* If loss probability is not more than 0.001, drop the packet */
	if ((((float) nloss) / narr) <= 0.001) {
	  nloss++;
	}
	else {
      q += 1;
  ...
  /* buffer more than 90% full; packet is dropped */
    nloss++;
  }
}
